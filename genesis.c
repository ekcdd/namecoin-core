...
int main(int argc, char *argv[])
{
	...
	int32_t nBits2 = 486604799; 
	// This is basically how I believe the size of the nBits is calculated
	if(nBits2 <= 255)
	{
		transaction->scriptSig[scriptSig_pos++] = 0x01;
		transaction->scriptSig[scriptSig_pos++] = (uint8_t)nBits2;
	}
	else if(nBits2 <= 65535)
	{
		transaction->scriptSig[scriptSig_pos++] = 0x02;
		memcpy(transaction->scriptSig+scriptSig_pos, &nBits2, 2);
		scriptSig_pos+=2;
	}	
	else if(nBits2 <= 16777215)
	{
		transaction->scriptSig[scriptSig_pos++] = 0x03;
		memcpy(transaction->scriptSig+scriptSig_pos, &nBits2, 3);
		scriptSig_pos+=3;
	}
	else //else if(nBits <= 4294967296LL)
	{
		transaction->scriptSig[scriptSig_pos++] = 0x04;
		memcpy(transaction->scriptSig+scriptSig_pos, &nBits2, 4);
		scriptSig_pos+=4;
	}
